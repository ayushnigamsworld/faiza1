...
- Documented build, test, validation, and run steps for clarity.
...
Or point to a specific data source. The server eagerly loads the file you provide at startup, so you can point it at any JSON transaction payload that matches `transactions.json`'s shape:
```bash
./main --transactions path/to/transactions.json
```

If the file cannot be found or a transaction is missing required fields (IDs, amounts, PAN, timestamps, etc.), startup will fail with a clear error. Successful startup logs which file was loaded and how many records were ingested.
